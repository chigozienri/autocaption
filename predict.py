...
class Predictor(BasePredictor):
    ...
    def predict(
        ...
        subs_position: str = Input(description="Subtitles position",  choices=["bottom75", "center" , "top", "bottom", "left", "right"], default="bottom75" ),
        color: str = Input(description="Caption color", default="white"),
        v_type: str = Input(description="Video ratio", choices=["9x16", "other aspect ratio"], default="other aspect ratio"),
        ...
        MaxChars: int = Input(description="Max Characters space for subtitles", default=20),
        opacity: float = Input(description="Opacity for the subtitles background", default=0.0),
    ) -> Path:
        ...
        outputfile = autocaption.add_subtitle(videofilename, audiofilename, v_type, subs_position, highlight_color, fontsize, opacity, MaxChars , color , wordlevel_info)
